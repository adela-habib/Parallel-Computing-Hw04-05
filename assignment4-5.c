...
{
  if(mpi_myrank==0){printf("INSIDE SIM_TICK.\n");}
  ...
  for(int i = 0; i < rows_per_thread; i++)
    {    
      ...
      // if(mpi_myrank==0){printf("Allocated space for temp_cells.\n");}
	  ...
	  if (curr_row == 0) //top thread row - use top ghost row
            {
	      //if(mpi_myrank==0){printf("I am in the top row.\n");}

	      /*
	      if(mpi_myrank==0)
		{
		  printf("cell to the left:  %d\n", my_rows[curr_row][left]);
		  printf("cell to the right: %d\n", my_rows[curr_row][right]);
		  printf("cell above:        %d\n", top_ghost_row[curr_col]);
		  printf("cell below:        %d\n", my_rows[down][curr_col]);
		  printf("cell up and left:  %d\n", top_ghost_row[left]);
		  printf("cell up and right: %d\n", top_ghost_row[right]);
		  printf("cell down left:    %d\n", my_rows[down][left]);
		  printf("cell down right:   %d\n", my_rows[down][right]);
		}
	      */
		...
		top_ghost_row[left] + top_ghost_row[right] + my_rows[down][left] + my_rows[down][right];
	      ...
	      //if(mpi_myrank==0){printf("I finished counting neighbors for top row.\n");}
            }
	  ...
	  //if(mpi_myrank==0){printf("Finished counting live neighbors.\n");}
...
void *run_simulation(void *void_pthread_id_ptr)
{
  ...
  for(int t = 0; t < num_ticks; t++)
      ...
      if ( (num_pthreads == 0) | (num_pthreads == 1) )
	{
	  ...
	  if (mpi_myrank==0){printf("There are only 0 or 1 threads.\n");}
	...
	{
	  if(mpi_myrank==0){printf("There are multiple threads. Inside run_sim for threads > 0\n");}
	  if(mpi_myrank==0){printf("TESTING HERE. PTHREADID = %d\n", pthread_id);}
	  ...
	  if(mpi_myrank==0){printf("Updated universe\n");}
      ...
      if(mpi_myrank==0){printf("Freed temporary universe.\n");}
      
      //Use pthread barrier to make sure each thread finishes a tick before moving on
      if(num_pthreads > 1) 
	{
	  if(mpi_myrank==0){printf("BEFORE CALLING PTHREAD BARRIER.\n");}
	  pthread_barrier_wait(&pbarrier);
	}
      if(mpi_myrank==0){printf("Finished pthread barrier.\n");}

      if(mpi_myrank==0){printf("FINISHED 1 TICK.\n");}
